...
WHERE DATE(Session.date) >= '$currentDate' AND Genre_idGenre = 6";

        $statement = $pdo->prepare($query);
        $statement->execute();
        $dances = $statement->fetchAll(PDO::FETCH_ASSOC);
        ?>

        <?php $dances = array_map("unserialize", array_unique(array_map("serialize", $dances))); ?>
        <?php foreach ($dances as $dance) { ?>
            <div class="container-fluid">
                <div class="row">
                <div class="col-2"></div>
                    <div class="col-lg-4">
                        <img src="Cover/<?php echo $dance['cover']; ?>" class="img-fluid">
                    </div>
                    <div class="event col col-lg-4">
                        <h1><?php echo $dance['event']; ?></h1>
                        <div class="row">
                            <div class="col">
                                <?php echo $dance['description']; ?>
                            </div>
                        </div>
                        <div class="col">
                            <form class="btn-position-right" method="GET" action="product.php" name="product">
                                <input type="hidden" name="idEvent" value="<?php echo $dance["idEvent"];?>" />
